    ...
    static pull(...selectors) {
        ...
        for (const selector of selectors) {
            if(typeof selector === 'object'){ // Custom Element
                ...
                list.push({
                    selected: selector,
                    $: [...document.getElementsByClassName(selector.substr(1))].map(Element.ifTemplateThenElement)
                })
            } else if (selector.startsWith("#")) { // get an element by id
                list.push({
                    selected: selector,
                    $: Element.ifTemplateThenElement(document.getElementById(selector.substr(1)))
                })
            } else {
