...
import {Element as ElementHandler } from "../utils/Handlers.js";
...
class Collector {
    #markReturnPointAsSubscriber

    constructor(markReturnPointAsSubscriber){
        this.#markReturnPointAsSubscriber = markReturnPointAsSubscriber
    }
    ...
    finish(){
        ...
        return this.#markReturnPointAsSubscriber
    }
}

class Subscriber {
    
    // value, textContent, getAttribute/setAttribute, Event(on, off)
    #pubs;
    #parsedData;
    #currentPub;
    ...
    constructor(...pubs) {
        //check if duplicated
        const originSize = pubs.length;
        const idCollections = pubs.map(pub=>pub.getId());
        if(originSize !== new Set(idCollections).size){
            throw new RangeError(`there are duplicated ids, please check the <Publisher> instance setting`)
        }
    ...
    from(id){
        this.#currentPub = this.#pubs.find(pub=>pub.getId() === id);
        return this;
    }
    ...
    el(selector){
        if (ElementHandler.checkSelectorString(selectors)) {
            throw new SyntaxError(`the ${selector} doesn't match identity format of element object`);
        }
    }
